...
export async function renderGallery() {
  ...
  const groups=[{id:'chinese-cases',title:'中文案例',items:cases.filter(c=>c.language==='zh')},{id:'international-cases',title:'其他语言案例',items:cases.filter(c=>c.language!=='zh')}].filter(g=>g.items.length);
  out+=groups.map(g=>`[${g.title}（${g.items.length}）](#${g.id})`).join(' · ')+'\n\n';
  for(const group of groups) {
    out+=`<a id="${group.id}"></a>\n\n## ${group.title}\n\n`;
    out+=group.items.map(c=>`[${md(c.title)}](#${c.id})`).join(' · ')+'\n\n';
